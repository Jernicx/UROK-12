...
GLuint IBO; //дл€ сохранени€ индекса вершин 
...
static void RenderSceneCB()
{
    ...
    glDrawElements(GL_TRIANGLES, 12, GL_UNSIGNED_INT, 0);// отрисовка трекгольника 
...
{
    Vector3f Vertices[4];// массив из трех элементов 
    Vertices[0] = Vector3f(-1.0f, -1.0f, 0.5773f);// 3 вершины дл€ треугольника
...
{
    unsigned int Indices[] = { 0, 3, 1, //Ѕуфер индексов заполн€етс€ с помощью массива индексов
                               1, 3, 2,
                               2, 3, 0,
                               0, 2, 1 };

    glGenBuffers(1, &IBO); // заполн€ем буфер индексов использу€ массив индексов.
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO);
